...
#include <testrobot_description_plugins/HydrodynamicModel.hh>

namespace gazebo
{
//this class inherits BuoyantObject class
  ...
  double _time, const math::Vector3 &_flowVelWorld)
{
  ...
  if ( this->debugFlag )
  {
    ...
    this->StoreVector(testrobot_DAMPING_FORCE, Vec3dToGazebo(damping.head<3>()));
    this->StoreVector(testrobot_DAMPING_TORQUE, Vec3dToGazebo(damping.tail<3>()));

    this->StoreVector(testrobot_ADDED_MASS_FORCE, Vec3dToGazebo(added.head<3>()));
    this->StoreVector(testrobot_ADDED_MASS_TORQUE, Vec3dToGazebo(added.tail<3>()));

    this->StoreVector(testrobot_ADDED_CORIOLIS_FORCE, Vec3dToGazebo(cor.head<3>()));
    this->StoreVector(testrobot_ADDED_CORIOLIS_TORQUE, Vec3dToGazebo(cor.tail<3>()));
  }
}
