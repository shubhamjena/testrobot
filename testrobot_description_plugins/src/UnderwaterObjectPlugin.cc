...
#include <testrobot_description_plugins/Def.hh>
  ...
  HydrodynamicModelPtr _hydro)
{
  ...
  _hydro->SetStoreVector(testrobot_DAMPING_FORCE);
  _hydro->SetStoreVector(testrobot_DAMPING_TORQUE);
  _hydro->SetStoreVector(testrobot_ADDED_CORIOLIS_FORCE);
  _hydro->SetStoreVector(testrobot_ADDED_CORIOLIS_TORQUE);
  _hydro->SetStoreVector(testrobot_ADDED_MASS_FORCE);
  _hydro->SetStoreVector(testrobot_ADDED_MASS_TORQUE);
}
...
//the below funtions are for debugging purposes
  ...
  physics::LinkPtr _link)
{
  if (this->models.count(_link))
  {
    ...
    force = this->models[_link]->GetStoredVector(testrobot_ADDED_MASS_FORCE);
    torque = this->models[_link]->GetStoredVector(testrobot_ADDED_MASS_TORQUE);

    this->GenWrenchMsg(force, torque, msg);
    this->hydroPub[_link->GetName() + "/added_mass"]->Publish(msg);

    // Publish wrench generated by the acceleraton of fluid aroung the object
    force = this->models[_link]->GetStoredVector(testrobot_DAMPING_FORCE);
    torque = this->models[_link]->GetStoredVector(testrobot_DAMPING_TORQUE);

    this->GenWrenchMsg(force, torque, msg);
    this->hydroPub[_link->GetName() + "/damping"]->Publish(msg);

    // Publish wrench generated by the acceleraton of fluid aroung the object
    force = this->models[_link]->GetStoredVector(testrobot_ADDED_CORIOLIS_FORCE);
    torque = this->models[_link]->GetStoredVector(testrobot_ADDED_CORIOLIS_TORQUE);
